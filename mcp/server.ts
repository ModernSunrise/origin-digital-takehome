// DevHub MCP server — exposes the event API as agent tools over stdio, importing the
...
async function main(): Promise<void> {
  ...
  console.error('DevHub MCP server running on stdio');
}

main().catch((error) => {
  console.error('Failed to start DevHub MCP server:', error);
  process.exit(1);
});
